...
# ==========================================
st.set_page_config(page_title="Network RCA System (Tiered)", page_icon="🛡️", layout="wide")
...
RECOMMENDATION_MAP = {
    ...
    'UPSTREAM_FAILURE': {
        ...
        'actions': ['Validasi status kabel Uplink/WAN (ether1).', 'Lakukan Ping ke Gateway Public (e.g., 8.8.8.8).', 'Koordinasi dengan penyedia ISP.']
    },
    'DDoS': {
        'title': 'DDoS Attack Pattern',
        'desc': 'Lonjakan trafik tidak wajar yang mengindikasikan serangan flood.',
        'actions': ['Analisis menu Firewall > Connections.', 'Terapkan Filter Rule untuk drop IP Source mencurigakan.']
    },
    'BROADCAST_STORM': {
        'title': 'L2 Loop / Broadcast Storm',
        'desc': 'Indikasi looping pada jaringan Layer-2 (Switching loop).',
        'actions': ['Identifikasi kabel yang menghubungkan antar-switch (loop).', 'Pastikan RSTP/STP aktif pada Bridge.']
    }
}
...
STOPWORDS = {
    ...
    'state', 'changed', 'ospf', 'ospf-1', 'router-id', 'area', 'area-0', 'neighbor', 'election',
    'version', 'instance', 'created', 'broadcast', 'loopback', 'dr', 'bdr', 'me', 'other',
    'loading', 'full', 'exchange', 'done', 'established', 'init', 'twoway', 'address', 'ip'
}
...
def process_chunk_aggregation(chunk_df, rules_df):
    ...
    GENERIC_KEYWORDS = {'interface', 'link', 'ethernet', 'port', '0x0800', 'udp', 'admin', 'bridge', 'proto', 'icmp', 'type', 'code'}
    ...
    for idx, row in chunk_df.iterrows():
        ...
        # --- 1. PRIORITAS: DETEKSI POLA KHUSUS (HARDCODED) ---
        if "internet connection lost" in msg.lower() or "8.8.8.8 rto" in msg.lower():
            diag, prio, evidence = 'UPSTREAM_FAILURE', 'FATAL', {'internet', 'lost', 'ping'}
        ...
        # --- 2. SECONDARY: ASSOCIATION RULES (Hanya jika belum terdeteksi pola di atas) ---
        if not diag:
            best_rule = None
            for _, rule in rules_df.iterrows():
                ...
                if rule['antecedents'].issubset(tokens):
                    ...
                    if best_rule is None or len(rule['antecedents']) > len(best_rule['antecedents']):
                        best_rule = rule
            ...
            if best_rule is not None:
                ...
                prio = "FATAL" if lift_val >= 6.0 else "CRITICAL" if lift_val >= 3.0 else "WARNING"
        ...
        if diag:
            ...
            if diag not in st.session_state['issues']:
                st.session_state['issues'][diag] = {
                    ...
                    'last_seen': row.get('time', '-'), 'evidence': set(), 'logs': [], 'lift': 0
                }
            ...
            if len(issue['logs']) < 50:
                issue['logs'].append({'Timestamp': row.get('time', '-'), 'Router': row.get('source_router', 'Unknown'), 'Message': msg})
...
# ==========================================
st.title("🛡️ Network Root Cause Analysis")
uploaded_file = st.file_uploader("Upload Log File (CSV)", type=['csv'])

if uploaded_file:
    # Load Rules
    rules_df = pd.read_csv('Data/rules/Rules_fix_v2.csv')
    rules_df = rules_df.rename(columns={'Root Cause (Gejala)': 'antecedents', 'Impact (Akibat)': 'consequents', 'Lift Ratio': 'lift'})
    
    # Pre-parse rules
    rules_df['final_diagnosis'] = rules_df['consequents'].apply(map_diagnosis)
    rules_df['antecedents'] = rules_df['antecedents'].apply(lambda x: set(ast.literal_eval(x)) - STOPWORDS)
    ...
    if st.button("Start Analysis"):
        ...
        m1.metric("Anomalies Found", len(st.session_state['issues']))
        
        # Display Cards
        for diag, data in sorted(st.session_state['issues'].items(), key=lambda x: x[1]['priority']):
            ...
            with st.expander("View Details"):
                ...
                for a in info['actions']: st.write(f"- {a}")
